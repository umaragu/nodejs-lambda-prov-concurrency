...
async function start() {
  let handler
  try {
	await initializeHook(process.argv[2]);
    handler = getHandler()
  } catch (e) {
    await initError(e)
...
async function initializeHook(method) {
	...
	const userHandler = app[method]
  
	if (userHandler == null) {
	  throw new Error(`Handler '${method}' missing on module '${modulePath}'`)
	} else if (typeof userHandler !== 'function') {
	  throw new Error(`Handler '${method}' from '${modulePath}' is not a function`)
	}
	await userHandler();
